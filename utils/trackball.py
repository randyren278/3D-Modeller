...
# -----------------------------------------------------------------------------
def _q_add(q1,q2):
    print(f"Trackball: _q_add called with q1={q1}, q2={q2}")
    t1 = _v_mul(q1, q2[3])
    t2 = _v_mul(q2, q1[3])
    t3 = _v_cross(q2, q1)
    tf = _v_add(t1, t2)
    ...
    return tf
def _q_mul(q, s):
    print(f"Trackball: _q_mul called with q={q}, s={s}")
    return [q[0]*s, q[1]*s, q[2]*s, q[3]*s]
def _q_dot(q1, q2):
    print(f"Trackball: _q_dot called with q1={q1}, q2={q2}")
    return q1[0]*q2[0] + q1[1]*q2[1] + q1[2]*q2[2] + q1[3]*q2[3]
def _q_length(q):
    print(f"Trackball: _q_length called with q={q}")
    return math.sqrt(_q_dot(q,q))
def _q_normalize(q):
    print(f"Trackball: _q_normalize called with q={q}")
    try:                      
        return _q_mul(q,1.0/_q_length(q))
    except ZeroDivisionError: 
        return q
def _q_from_axis_angle(v, phi):
    print(f"Trackball: _q_from_axis_angle called with v={v}, phi={phi}")
    q = _v_mul(_v_normalize(v), math.sin(phi/2.0))
    q.append(math.cos(phi/2.0))
    return q
def _q_rotmatrix(q):
    print(f"Trackball: _q_rotmatrix called with q={q}")
    m = [0.0]*16
...
class Trackball(object):
    ...
    def __init__(self, theta=0, phi=0, zoom=1, distance=3):
        ...
        print(f"Trackball: Initializing with theta={theta}, phi={phi}, zoom={zoom}, distance={distance}...")
    ...
    def zoom_to (self, x, y, dx, dy):
        ...
        self._zoom = self._zoom-5*dy/height
    ...
    def _get_matrix(self):
        print("Trackball: Getting matrix...")
        return self._matrix
    matrix = property(_get_matrix, doc='''Model view matrix transformation (read-only)''')

    def _get_zoom(self):
        print("Trackball: Getting zoom...")
        return self._zoom
    def _set_zoom(self, zoom):
        print(f"Trackball: Setting zoom to {zoom}...")
        self._zoom = zoom
    ...
    def _get_distance(self):
        print("Trackball: Getting distance...")
        return self._distance
    def _set_distance(self, distance):
        print(f"Trackball: Setting distance to {distance}...")
        self._distance = distance
        if self._distance < 1: self._distance = 1
    distance = property(_get_distance, _set_distance, doc='''Scene distance from point of view''')

    def _get_theta(self):
        print("Trackball: Getting theta...")
        self._theta, self._phi = self._get_orientation()
        return self._theta
    def _set_theta(self, theta):
        print(f"Trackball: Setting theta to {theta}...")
        self._set_orientation(math.fmod(theta,360.0), math.fmod(self._phi,360.0))
    theta = property(_get_theta, _set_theta, doc='''Angle (in degrees) around the z axis''')

    def _get_phi(self):
        print("Trackball: Getting phi...")
        self._theta, self._phi = self._get_orientation()
        return self._phi
    def _set_phi(self, phi):
        print(f"Trackball: Setting phi to {phi}...")
    ...
    def _get_orientation(self):
        ...
        print("Trackball: Getting orientation...")
    ...
    def _set_orientation(self, theta, phi):
        ...
        print(f"Trackball: Setting orientation to theta={theta}, phi={phi}...")
        self._theta = theta
        self._phi = phi
    ...
    def _project(self, r, x, y):
        ...
        '''
        print(f"Trackball: Projecting x={x}, y={y}, r={r}...")
        d = math.sqrt(x*x + y*y)
    ...
    def _rotate(self, x, y, dx, dy): 
        ...
        '''
        print(f"Trackball: Rotating from ({x}, {y}) by ({dx}, {dy})...")
